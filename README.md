...
1. Auth.js (A Chave): Gerencia exclusivamente o protocolo de autenticação. Ele gera as sessões que permitem o acesso de múltiplas identidades sem a necessidade de re-login constante.

2. List-group.js (O Olheiro): Realiza o mapeamento de superfície. Ele identifica todos os grupos e canais aos quais as contas têm acesso, definindo os alvos da operação.

3. Scout.js (O Analista): Este é o cérebro da indexação. Ele varre as mensagens em busca de .txt, mas com um diferencial crítico: ele mapeia o Data Center (DC) de origem de cada mensagem, permitindo que o download seja otimizado para a região geográfica correta.

4. Downloader.js (O Executor): Responsável pelo I/O pesado. Ele consome a fila do banco de dados e realiza o download físico dos ativos.

5. Manager.js (O Gerente): Atua na camada de controle (Control Plane). Ele orquestra o rodízio de proxies e sessões, garantindo que a operação nunca pare por limites de um único IP ou conta.

#### Mapa Mental e Fluxo de Dados:
...
Problema: Rate Limiting (FloodWait)
...
Problema: Fragmentação de Data Centers (DC)
